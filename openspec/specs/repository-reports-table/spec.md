...
Every **`GET /api/v1/reports`** from this tab SHALL include **`inputType=rpm`** alongside paging, sorting, and other active filters.
...
- **THEN** **Package** shows **`name`-`version`-`release`**
...
The ID column SHALL display `report.id` as a link to the report page (component route: `/reports/component/{cveId}/{report.id}`; product route: `/reports/product/{productId}/{cveId}/{report.id}`). The **Date Requested** column SHALL display `metadata.submitted_at` when present, in the format "DD Month YYYY, HH:MM:SS AM/PM"; when `metadata.submitted_at` is missing, the cell SHALL display "-". The **Date Completed** column SHALL display `report.completedAt` in the same format.

The table SHALL display a single **Finding** column (no separate "Analysis state" or "ExploitIQ Status" column). The Finding cell SHALL show, per row: if the report's analysis state is **completed**, the ExploitIQ status (Vulnerable, Not vulnerable, or Uncertain) from the vulnerability justification with the same label colors as **reports-table**; if the report's analysis state is **pending**, **queued**, or **sent**, "In progress" using the shared InProgressStatus component (grey outline label, InProgressIcon); if the report's analysis state is **expired** or **failed**, "Failed" using the shared FailedStatus component (grey filled label, ExclamationCircleIcon).

#### Scenario: Standard layout on report page and single-repositories tab

- **WHEN** a user views the table on the SBOM report page or **`/reports/single-repositories`**
- **THEN** the column set, ID link routes, and date formats match this requirement

#### Scenario: Finding cell by analysis state

- **WHEN** a user views a row whose analysis state is completed, in progress (pending/queued/sent), or failed (expired/failed)
- **THEN** the Finding cell shows the corresponding ExploitIQ label, InProgressStatus, or FailedStatus per this requirement
...
#### Scenario: Finding filter selection and clearing

- **WHEN** a user selects a Finding value or clears the filter
- **THEN** the table sends the mapped backend parameters for the selection, or omits finding-based status/exploitIqStatus parameters when cleared
...
The repository reports table SHALL use backend pagination via **`GET /api/v1/reports`** with **`page`**, **`pageSize`**, and list parameters appropriate to layout (**`inputType=repository`** on **`/reports/single-repositories`**, **`inputType=rpm`** on **`/reports/rpm`**; both values imply **no** **`metadata.product_id`** per **reports-input-type**, plus **`gitRepo`** (repository tab), **`rpmPackage`** where used (RPM tab), **`vulnId`**, **aggregated Finding parameters** as implemented). The table SHALL display pagination controls and allow navigating between pages.

The table SHALL display a loading skeleton only on the initial load when the table is first displayed; the skeleton SHALL show placeholder rows matching the table structure and be replaced by actual data once the initial fetch completes. When users change sort order or filters, the existing table data SHALL remain visible while new data loads in the background; the table SHALL update with new data once the API call completes, without showing the skeleton.

#### Scenario: Backend pagination

- **WHEN** a user views the repository reports table or navigates pages
- **THEN** pagination uses **`GET /api/v1/reports`** with **`page`**, **`pageSize`**, and layout-appropriate list parameters

#### Scenario: Loading skeleton only on initial display

- **WHEN** the table is first displayed
- **THEN** a structure-matching skeleton appears until the initial fetch completes
- **WHEN** the user subsequently changes sort order or any filter value
- **THEN** existing rows remain visible without a skeleton until refreshed data replaces them
...
The repository reports table SHALL be sorted by default in descending order by submitted_at (most recently requested first). Sorting SHALL be performed server-side via the reports API (e.g. `sortBy=submittedAt:DESC`). The **Date Requested** column header SHALL reflect the active sort when applicable.

#### Scenario: Default sort on first load

- **WHEN** a user first views the repository reports table before changing sort
- **THEN** rows are ordered by **`submitted_at`** descending via server-side API sort parameters
...
- **WHEN** a row's report has **`componentDependencyTriageFailed` true**
- **THEN** a warning icon appears to the left of that row's repository URL
- **AND** focusing or hovering the icon shows the tooltip text defined above

#### Scenario: No warning when triage did not fail for that component

- **WHEN** a row's report has **`componentDependencyTriageFailed` false**, undefined, or absent
